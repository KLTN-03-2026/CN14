...
const { latestRevenuee } = require("../../../../helpers/latest-revenue");
...
module.exports.categoryGrowth = async (req, res) => {
  ...
  const countCategoryCurrent = await Category.countDocuments({
    createdAt: {
      $gte: lastMonth,
      $lt: currentMonth
    },
  ...
  const countCategoryLast = await Category.countDocuments({
    createdAt: {
      $gte: lastMonth2,
      $lt: lastMonth
    },
...
module.exports.latestRevenue = async (req, res) => {
  try {
    ...
    for (var i = 0; i < 4; i++) {
      ...
      const revenue = await latestRevenuee(`${year}-${month}`);
